...
export default class extends BaseSchema {
  ...
  public async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table.enum('pricingType', ['fixed', 'discount_percentage', 'discount_fixed']).defaultTo('fixed')
      ...
      table.boolean('trackInventory').defaultTo(true) // Track bundle inventory separately
      table.integer('stockQuantity').defaultTo(0) // Bundle stock if tracked separately

      // Visibility
      table.boolean('isVisible').defaultTo(true) // Show component products to customers
